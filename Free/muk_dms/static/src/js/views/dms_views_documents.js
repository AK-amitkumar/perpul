...
flectra.define('muk_dms_views.documents', function(require) {
"use strict";
...
var DocumentTreeView = Widget.extend(ControlPanelMixin, {
    ...
    render: function() {
        ...
        this.$tree = this.$el.find('.oe_document_tree').jstree({
        					...
        					callback.call(this, _.map(settings, function (setting) {
        						...
        						return {
        							...
        							icon: "fa fa-database",
        	  	        			type: "settings",
        	  	        			data: {
        		        				flectra_id: setting.id,
        		        				flectra_model: "muk_dms.settings",
        		        			},
    		...
    		if(data.node.data && data.node.data.flectra_model === "muk_dms.directory") {
    ...
    _load_node(node) {
    	var self = this;
    	var result = $.Deferred();
    	if(node.data && node.data.flectra_model === "muk_dms.directory") {
    		...
    		this._rpc({
                ...
                domain: [
                	['parent_directory', '=', node.data.flectra_id]
                ],
                model: 'muk_dms.directory',
                method: 'search_read',
                context: session.user_context,
    		...
    		this._rpc({
                ...
                domain: [
                	['directory', '=', node.data.flectra_id]
                ],
                model: 'muk_dms.file',
                method: 'search_read',
                context: session.user_context,
    ...
    _node_directory(directory) {
    	return {
			...
			type: "directory",
			data: {
				flectra_id: directory.id,
				flectra_model: "muk_dms.directory",
				name: directory.name,
    ...
    _check_callback: function (operation, node, parent, position, more) {
    	if(operation === "copy_node" || operation === "move_node") {
    		...
    		if(parent.data && parent.data.flectra_model === "muk_dms.settings") {
    			return false;
            }
    		// prevent moving a child to a file
    		if(parent.data && parent.data.flectra_model === "muk_dms.file") {
    			return false;
            }
    	}
    	if(operation === "move_node") {
    		...
    		if(node.data && parent.data) {
    			...
    			_.each(parent.children, function(child, index, children) {
    				...
    				if(child_node.data && child_node.data.flectra_model === parent.data.flectra_model) {
    ...
    _preview_node: function(node) {
    	var self = this;
    	if(node.data && node.data.flectra_model === "muk_dms.file") {
    		PreviewHelper.createFilePreviewContent(node.data.flectra_id, self).then(function($content) {
    			self.$el.find('.oe_document_preview').html($content);
    		});
    	} else if(node.data && node.data.flectra_model === "muk_dms.directory") {
    	...
    	if(this.$tree) {
    		...
    		if(node.data) {
    			this.do_action({
    	            ...
    	            res_model: node.data.flectra_model,
    	            res_id: node.data.flectra_id,
    	            views: [[false, 'form']],
    ...
    _move_node: function(node, parent) {
    	...
    	if(this.$tree) {
    		...
    		var vals = {};
    		if(node.data && parent.data && 
    				node.data.flectra_model === "muk_dms.file") {
    			vals.directory = parent.data.flectra_id;
    		} else if(node.data && parent.data && 
    				node.data.flectra_model === "muk_dms.directory") {
    			vals.parent_directory = parent.data.flectra_id;
    		}
    		this._rpc({
                model: node.data.flectra_model,
                method: 'write',
                args: [node.data.flectra_id, vals],
                context: session.user_context,
    		}).done(function() {
    ...
    _copy_node: function(node, original, parent) {
    	...
    	if(this.$tree) {
    		...
    		if(original.data && parent.data && 
    				original.data.flectra_model === "muk_dms.file") {
    			vals.directory = parent.data.flectra_id;
    		} else if(original.data && parent.data && 
    				original.data.flectra_model === "muk_dms.directory") {
    			vals.parent_directory = parent.data.flectra_id;
    		}
    		this._rpc({
                model: original.data.flectra_model,
                method: 'copy',
                args: [original.data.flectra_id, vals],
                context: session.user_context,
    		}).done(function(copy_id) {
    			node.data = original.data;
    			node.id = original.data.flectra_model.split(".") + "_" + copy_id;
    ...
    _rename_node: function(node, new_name, old_name) {
    	...
    	if(this.$tree && node.data) {
    		this._rpc({
                model: node.data.flectra_model,
                method: 'write',
                args: [node.data.flectra_id, {'name': new_name}],
                context: session.user_context,
    		}).done(function() {
    	...
    	if(this.$tree && node.data) {
	    	if(type == "muk_dms.file") {
	    		context = $.extend(session.user_context, {
	    			default_directory: node.data.flectra_id
	            });
	    	} else if(type == "muk_dms.directory") {
	    		context = $.extend(session.user_context, {
	    			default_parent_directory: node.data.flectra_id
	            });
	    	}
	    	this.do_action({
	    		type: 'ir.actions.act_window',
    ...
    _load_contextmenu: function(node, callback) {
    	...
    	if(node.data) {
    		...
    		if(node.data.flectra_model === "muk_dms.directory") {
    			menu = widget._load_contextmenu_directory(this, node, menu);
    		} else if(node.data.flectra_model === "muk_dms.file") {
