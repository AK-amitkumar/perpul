...
class CenitIntegrationSettings(models.TransientModel):
    ...
    def get_values_user(self):
        user = self.env['ir.config_parameter'].get_param(
            'flectra_cenit.shipwire.user', default=None
        )
        return {'user': user or ''}

    def get_values_passwd(self):
        passwd = self.env['ir.config_parameter'].get_param(
            'flectra_cenit.shipwire.passwd', default=None
        )
    ...
    def set_values(self):
        ...
        for record in self.browse(self.ids):
            config_parameters.set_param (
                'flectra_cenit.shipwire.user', record.user or ''
            )
        for record in self.browse(self.ids):
            config_parameters.set_param (
                'flectra_cenit.shipwire.passwd', record.passwd or ''
            )
