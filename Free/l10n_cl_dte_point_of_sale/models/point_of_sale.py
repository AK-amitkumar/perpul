...
&field=sii_xml_request&id=%s&filename=%s' % (self.id,filename ),
    ...
    def _process_order(self, order):
        ...
        order_id.sequence_number = order['sequence_number'] #FIX flectra bug
...
'<Documento_ID>', doc_id).replace('</Documento_ID>', '</Documento>')
    ...
    def ask_for_dte_status(self):
        ...
        if not self.sii_send_ident:
            raise UserError('No se ha enviado aún el documento, aún está en cola de envío interna en flectra')
    ...
    def _create_account_move_line(self, session=None, move=None):
        ...
        for order in self.filtered(lambda o: not o.account_move or o.state == 'paid'):
            ...
            #@TODO testear si esto ya repara los problemas de redondeo original de flectra
