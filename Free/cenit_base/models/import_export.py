...
from flectra import models, fields, http, api, exceptions, tools, _
...
class ImportExport(models.TransientModel):
    ...
    def import_data_types(self):
        ...
        for data in json_data:
            flectra_model = data['model']
            namespace = data['namespace']
            schema = data['schema']

            domain = [('model', '=', flectra_model)]
            candidates = irmodel_pool.search(domain)
            if not candidates:
                raise exceptions.MissingError(
                    "There is no %s module installed" % flectra_model
                )
            flectra_model = candidates.id
            ...
            vals = {'name': data['name'], 'model': flectra_model, 'namespace': namespace, 'schema': schema}
