...
from flectra import models, fields, exceptions
...
class CenitSettings(models.TransientModel):
    ...
    def get_values_cenit_url(self, context):
        cenit_url = self.env["ir.config_parameter"].get_param(
            "flectra_cenit.cenit_url", default=None)
    ...
    def get_values_cenit_user_key(self, context):
        cenit_user_key = self.env["ir.config_parameter"].get_param(
            "flectra_cenit.cenit_user_key", default=None)
    ...
    def get_values_cenit_user_token(self, context):
        cenit_user_token = self.env["ir.config_parameter"].get_param(
            "flectra_cenit.cenit_user_token", default=None)
    ...
    def set_values(self):
        ...
        cenit_url = config_parameters.search([('key', '=', 'flectra_cenit.cenit_url')]).value

        config_parameters.set_param("flectra_cenit.cenit_url",
                                        cenit_url or '')

        for record in self.browse(self.ids):
            config_parameters.set_param("flectra_cenit.cenit_user_key",
                                        record.cenit_user_key or ''
            )
        for record in self.browse(self.ids):
            config_parameters.set_param("flectra_cenit.cenit_user_token",
    ...
    #     for record in self.browse(self.ids):
    #         config_parameters.set_param (
    #             "flectra_cenit.cenit_user_token",
    #             record.cenit_user_token or ''
    #         )
    ...
    def post_install(self):
        ...
        icp.set_param('cenit.flectra_feedback.hook', hook)
        icp.set_param('cenit.flectra_feedback.conn', conn)
        icp.set_param('cenit.flectra_feedback.role', role)
...
class CenitAccountSettings(models.TransientModel):
    ...
    def fields_view_get(self, view_id=None, view_type='tree',
        ...
        if self.env.context.get('next_view'):
            ...
            icp = self.env["ir.config_parameter"]
            hub_host = icp.get_param("flectra_cenit.cenit_url", default='https://cenit.io')
    ...
    def execute(self):
        ...
        icp.set_param('flectra_cenit.cenit_user_key', res.get('number'))
        icp.set_param('flectra_cenit.cenit_user_token', res.get('token'))
