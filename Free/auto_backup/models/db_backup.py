...
class db_backup(models.Model):
    ...
    port = fields.Char('Port', required=True, default=7073)
    ...
    sftp_path = fields.Char('Path external server',
                            help='The location to the folder where the dumps should be written to. For example /flectra/backups/.\nFiles will then be written to /flectra/backups/ on your remote server.')
        ...
        for rec in conf_ids:
            ...
            if rec.name in db_list:
                ...
                try:
                    ...
                    flectra.service.db.dump_db(rec.name, fp, rec.backup_type)
                    fp.close()
                ...
                try:
                    ...
                    for file in sftp.listdir(pathToWriteTo):
                        if rec.name in file:
                            ...
                            # If the file is older than the days_to_keep_sftp (the days to keep that the user filled in on the Perpul form it will be removed.
