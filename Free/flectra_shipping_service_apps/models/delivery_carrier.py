...
from flectra.addons.flectra_shipping_service_apps.tools import DomainVals
BasicAddress = ['name','email','phone','street','street2','city','zip','lang']
...
class DeliveryCarrier(models.Model):
    ...
    def wk_get_shipping_price_from_so(self, orders):
        ...
        result = self.with_context(self._context).get_shipping_price_from_so(orders)[0]
        return result
    @api.model
    ...
    uom_id = fields.Many2one(
        ...
        string='Perpul Product UoM',
        help='Shipping Equivalent UoM in Perpul'
    )
    delivery_uom = fields.Selection(
        selection=APIUoM,
        string='API UoM',
        help='Default UoM in Perpul'
    )
    ...
    def wk_get_product_package(self,line,packaging_id,partial_package=None):
        ...
        if qty_capacity and     qty_capacity<max_qty:
            ...
            if package_count:
                ...
                _logger.info("-Condition-B1-%r--%r==%r====%r"%([multi_pckg_max_qty],package_count,product_qty,max_qty))
    ...
    def get_shipment_address(self,entity):
        ...
        if entity.company_id:
            company_name =entity.company_id.name
