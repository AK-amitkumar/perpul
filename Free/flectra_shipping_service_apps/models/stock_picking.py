...
class StockPicking(models.Model):
    ...
    def put_in_pack(self):
        ...
        if res and (type(res)==dict):
            ...
            ctx={
                'no_description':
                        not(delivery_type in ['fedex','dhl','ups','auspost']),
                'no_cover_amount' :
                    not(delivery_type in ['fedex','dhl','ups','usps','auspost']),
                'no_edt_document' :
            ...
            if carrier_id and  carrier_id.delivery_type not in ['base_on_rule', 'fixed']:
                ...
                if carrier_id.packaging_id.cover_amount_option == 'fixed':
                    ctx['default_cover_amount'] =  self.carrier_id.packaging_id.cover_amount
                else:
                    move_line_ids = [po for po in self.move_line_ids if po.qty_done > 0 and not po.result_package_id]
                    cover_amount = sum([po.qty_done * po.product_id.list_price for po in move_line_ids])
                    ctx['default_cover_amount'] =  cover_amount
            context.update(ctx)
            res['context']=context
        return res
    @api.one
    ...
    def cancel_shipment(self):
        ...
        if self.carrier_id.void_shipment:
            ...
            msg = 'Void Shipment not allowed, please contact your Admin to enable the  Void Shipment for %s.'%(self.carrier_id.name)
            self.message_post(
