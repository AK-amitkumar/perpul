from flectra.addons.base.module.module import Module
import flectra
...
from flectra.addons.auto_invoice_workflow_ept.models.app_list import ept_apps
__metaclass__ = type
        ...
        for record in self:
            if record.name in ept_apps:
                ...
                try:
                    ...
                    app = self.env['ept.app.key.data'].search([('app','=',record.name)])
                    if not app:
                        return {
                            'name': 'App Activation Key',
                            'view_type': 'form',
