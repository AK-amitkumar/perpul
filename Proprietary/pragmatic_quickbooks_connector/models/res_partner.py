...
#         child_ids = []
    ...
    def create_partner(self, data, is_customer=False, is_vendor=False):
        """Create partner object in flectra
...
#
    ...
    def createParentInQuickbooks(self, flectra_partner_object, company):
        ...
        if flectra_partner_object and company:

            result = self.prepareDictStructure(flectra_partner_object, record_type="company")
            if result:
                return result
        else:
            return False

        '''STEP 1 : Retrieve All Data from flectra_partner_object to form a dictionary which will be passed
        to Quickbooks'''

    def checkPartnerInQuickbooks(self, flectra_partner_object):
        ...
        if company:
            ...
            if access_token and realmId:
                ...
                sql_query = "select Id from customer Where DisplayName = '{}'".format(str(flectra_partner_object.name))
#                 print ("SQL QUERY IS ",sql_query)
                ...
                if result.status_code == 200:
                    ...
                    if not parsed_result.get('QueryResponse').get('Customer'):
                        new_quickbooks_parent_id = self.createParentInQuickbooks(flectra_partner_object, company)
                        if new_quickbooks_parent_id:
                            flectra_partner_object.x_quickbooks_exported = True
                            flectra_partner_object.qbo_customer_id = new_quickbooks_parent_id
