...
from flectra.exceptions import RedirectWarning, ValidationError#,Warning
from flectra.addons.amazon_flectra_bridge.tools.tools import CHANNELDOMAIN
from flectra.addons.amazon_flectra_bridge.tools.tools import ProductIdType


from flectra.addons.amazon_flectra_bridge.tools.tools import prettify
from flectra.addons.amazon_flectra_bridge.tools.tools import get_fd
from flectra.addons.amazon_flectra_bridge.tools.tools import extract_list as EL
from flectra.addons.amazon_flectra_bridge.tools.tools import add_text as AT
from flectra.addons.amazon_flectra_bridge.tools.tools import FIELDS
from flectra.addons.amazon_flectra_bridge.tools.tools import JoinList as JL
from flectra.addons.amazon_flectra_bridge.tools.tools import extract_item as EI
from flectra.addons.amazon_flectra_bridge.tools.tools import MapId
from flectra.addons.amazon_flectra_bridge.tools.tools import Mapname
...
class MWSFeed(models.Model):
    ...
    def create(self,vals):
        try:
            vals['name'] = self.env.ref('amazon_flectra_bridge.feed_sequence').next_by_id()
        ...
        for template_id in template_ids:
            ...
            if len(variant_ids)>1:
                for message_id, product in enumerate(variant_ids,1):
                    ...
                    for mapping_id in product.channel_mapping_ids:
                ...
                full_image_url = '%s' % urlparse.urljoin(base_url,image_url),
                for mapping_id in object_id.channel_mapping_ids:
                    item = dict(
                        default_code =mapping_id.default_code,
                        url = full_image_url
                    )
    ...
    def post_order_fulfillment(self,picking_ids,channel_id,api_sdk=None):
        ...
        message_id = 1
        FulfillmentDate = fields.Datetime.from_string(fields.Datetime.now()).isoformat()
    ...
    def post_flectra_feed(self,object_ids,channel_id,feed_type,api_sdk=None):
        res = dict(
            message = '',
            feed_id = []
        )
    ...
    def submit_feed(self):
        ...
        for record in self:
            ...
            if record.feed_type =='product':
                ...
                price_feed_res=record.post_flectra_feed(product_tmpl_ids,channel_id,'price')
                message +=  price_feed_res.get('message')
                feed_id = price_feed_res.get('feed_id')
            elif record.feed_type=='inventory':
                inventory_feed_res=record.post_flectra_feed(product_tmpl_ids,channel_id,'inventory')
            ...
            for product_id in product_ids:
                ...
                if mapping_id:
                    ...
                    mapping_id = channel_id.create_product_mapping(
                        flectra_template_id = product_id.product_tmpl_id,
                        flectra_product_id = product_id,
                    ...
                    tmpl_mapping_id = channel_id.match_template_mappings(
                        ...
                        domain = [('flectra_template_id', '=', product_id.id)],
                    )
