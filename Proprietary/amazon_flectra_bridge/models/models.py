...
from flectra.addons.amazon_flectra_bridge.tools.tools import CHANNELDOMAIN,extract_item
from flectra.addons.amazon_flectra_bridge.tools.tools import FIELDS,MAPPINGDOMAIN,ProductIdType
from flectra.addons.amazon_flectra_bridge.tools.wk_mws_api import WKMWSAPI
from flectra.addons.flectra_multi_channel_sale.tools import DomainVals,MapId,JoinList
...
class MultiChannelSale(models.Model):
    ...
    def mws_ensure_report(self):
        ...
        if not report_id:
            ...
            #action_id = self.env.ref('amazon_flectra_bridge.action_reports').id
    ...
    def set_mws_import_product_cron(self):
        self.set_channel_cron(
            ref_name = 'amazon_flectra_bridge.cron_import_product_from_mws',
    ...
    def set_mws_import_order_cron(self):
        self.set_channel_cron(
            ref_name = 'amazon_flectra_bridge.cron_import_order_from_mws',
    ...
    def set_mws_import_order_status_cron(self):
        self.set_channel_cron(
            ref_name = 'amazon_flectra_bridge.cron_import_order_status_from_mws',
            active = self.mws_imp_order_status_cron
        )
    @api.onchange('mws_report_cron')
    def set_mws_report_cron(self):
        self.set_channel_cron(
            ref_name = 'amazon_flectra_bridge.cron_send_request_mws_report',
            active = self.mws_report_cron
        )
        self.set_channel_cron(
            ref_name = 'amazon_flectra_bridge.cron_send_generate_mws_report',
            active = self.mws_report_cron
        )
        self.set_channel_cron(
            ref_name = 'amazon_flectra_bridge.cron_send_receive_mws_report',
            active = self.mws_report_cron
        )
    ...
    def mws_order_status_update(self,order_ids):
        ...
        Feed.post_flectra_feed(order_ids,self, 'order')
