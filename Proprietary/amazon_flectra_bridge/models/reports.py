...
from flectra.addons.amazon_flectra_bridge.tools.tools import extract_item as ET
...
from flectra.addons.flectra_multi_channel_sale.tools import DomainVals,MapId
...
class MWSReports(models.Model):
    ...
    def create(self,vals):
        try:
            vals['name'] = self.env.ref('amazon_flectra_bridge.report_sequence').next_by_id()
    ...
    def mws_get_product_attribute_vals(self, node):
        ...
        if attributes:
            for attribute in attributes:
                ...
                temp_name = '%s_wkflectra_%s'%(name,store_attribute_id)
    ...
    def mws_import_attributes(self, node, channel_id):
        ...
        for name_id , val_list in vals.items():
            if name_id:
                name , store_attribute_id = name_id.split('_wkflectra_')
                attr_vals = dict(
                    name = name.strip(),
