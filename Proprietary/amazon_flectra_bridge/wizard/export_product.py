...
from flectra.exceptions import  UserError,RedirectWarning, ValidationError ,Warning
from flectra.addons.amazon_flectra_bridge import const
from flectra.addons.flectra_multi_channel_sale import tools as mtools
import logging
    ...
    def mws_submit_create_feed(self):
        ...
        for record in self:
            ...
            if not len(tmpl_ids):
                ...
                sync_vals['flectra_id'] = tmpl_ids.ids
                channel_id._create_sync(sync_vals)
                # Feed.post_flectra_feed(product_tmpl_ids, channel_id, 'image')
                Feed.post_flectra_feed(tmpl_ids,channel_id, 'inventory')
                Feed.post_flectra_feed(tmpl_ids,channel_id,'price')
                # Feed.post_flectra_feed(product_tmpl_ids, channel_id,'relationship')
    ...
    def mws_submit_update_feed(self):
        ...
        for record in self:
            ...
            if len(product_tmpl_ids):
                if mws_update_image:
                    inventory_feed_res =Feed.post_flectra_feed(product_tmpl_ids,channel_id,'image')
                    message +=  inventory_feed_res.get('message')
                    feed_ids += inventory_feed_res.get('feed_id')
                if mws_update_qty:
                    inventory_feed_res =Feed.post_flectra_feed(product_tmpl_ids,channel_id,'inventory')
                    message +=  inventory_feed_res.get('message')
                    feed_ids += inventory_feed_res.get('feed_id')
                if mws_post_product_price:
                    price_feed_res = Feed.post_flectra_feed(product_tmpl_ids,channel_id,'price')
                    message +=  price_feed_res.get('message')
                    feed_ids+= price_feed_res.get('feed_id')
                if mws_update_relationship:
                    price_feed_res = Feed.post_flectra_feed(product_tmpl_ids,channel_id,'relationship')
                    message +=  price_feed_res.get('message')
                    feed_ids+= price_feed_res.get('feed_id')
