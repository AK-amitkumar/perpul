...
class MultiChannelSkeleton(models.Model):
            ...
            if not exists:
                ...
                if journal_obj:
                    MapVals = {
                        ...
                        'flectra_journal': journal_id,
                        'flectra_journal_id': journal_id,
                        'channel_id': ChannelID.id,
                        'ecom_store': ChannelID.channel}
                    MapObj.create(MapVals)
            else:
                journal_id = exists[0].flectra_journal_id
        return {
    ...
    @api.model
    def _ConfirmFlectraOrder(self, order_obj):
        """ Confirms Perpul Order from E-Commerce
        @param order_id: Perpul/ERP Sale Order ID
        ...
        try:
            ...
            status_message += "Error in Confirming Order on Perpul: %s<br/>" % str(
                e)
            status = False
        finally:
            return {
    ...
    def set_order_shipped(self, order_id):
        """Cancel the order in Perpul via requests from XML-RPC
        @param order_id: Perpul Order ID
        ...
        try:
            ...
            if sale.state == 'draft':
                res = self._ConfirmFlectraOrder(sale)
    ...
    def create_order_invoice(self, order_obj):
        ...
        @param order_id: Perpul Order ID
        @return: a dictionary containig Perpul Invoice IDs and Status with Status Message
        """
        invoice_id = False
        status = True
    ...
    def SetOrderPaid(self, payment_data):
        """Make the order Paid in Perpul using store payment data
        ...
        if counter <= 1:
            ...
            status = False
            status_message = "<br/>Multiple validated Invoices found for the Perpul order. Cannot make Payment<br/>"
        return {
            'status_message': status_message,
            'status': status
        }
    ...
    def get_default_payment_method(self, journal_id):
        ...
        @params context : Must have key 'ecommerce' and then return payment payment method based on Perpul Bridge used else return the default payment method for Journal
    ...
    OrderState=False, CreateInvoice=False, InvoiceState=False, ShipOrder=False,**kwargs):
        ...
        if OrderState and OrderState in ['sale', 'done']:
            res = self._ConfirmFlectraOrder(OrderObj)
            status_message += res['status_message']
            status = res['status']
    ...
    def _SetFlectraOrderState(self, order_id, channel_id, feed_order_state='', payment_method=False,**kwargs):
        ...
        if channel_id and order_id and order_id.order_line:
            ...
            if order_state_ids:
                ...
                order_state = state_id.flectra_order_state
                create_invoice = state_id.flectra_create_invoice
                invoice_state = state_id.flectra_set_invoice_state
                ship_order = state_id.flectra_ship_order
...
class ChannelOrderStates(models.Model):
    ...
    rec_name = 'flectra_order_state'

    channel_state = fields.Char('Channel Order State')
    flectra_order_state = fields.Selection([
        ('draft', 'Quotation'),
        ('sale', 'Sale Order'),
        ('done', 'Done'),
        ('shipped', 'Shipped'),
    ...
    default_order_state = fields.Boolean(
        ...
        help="Default state if store state does not match the flectra state.")
    flectra_create_invoice = fields.Boolean(
        'Create Invoice', help="Create invoice in flectra.")
    flectra_set_invoice_state = fields.Selection([
        ('open', 'Open'), ('paid', 'Paid')],
        string='Set Invoice State',
        help="The field is used to retrieve orders that are in a specific state.")
    flectra_ship_order = fields.Boolean(string="Create Shippment")
    _sql_constraints = [
    ...
    @api.one
    @api.constrains('flectra_order_state', 'flectra_create_invoice', 'flectra_set_invoice_state')
    def _check_order_state(self):
        if self.flectra_order_state not in ['sale', 'done'] and self.flectra_create_invoice:
            raise ValidationError(
                "Invoice can not be created for a %s sale order." % (self.flectra_order_state))
        if (not self.flectra_create_invoice) and self.flectra_set_invoice_state:
