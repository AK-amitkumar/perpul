...
class ChannelCategoryMappings(models.Model):
	...
	flectra_category_id = fields.Integer(string='Perpul Category ID',required=True)
	...
	_sql_constraints = [
		...
		('channel_flectra_category_id_uniq',
		'unique(channel_id, flectra_category_id)',
		'Perpul category ID must be unique for channel category mapping!'),
	]
	@api.multi
	def unlink(self):
		for record in self:
	...
	def change_flectra_id(self):
		self.flectra_category_id = self.category_name.id
