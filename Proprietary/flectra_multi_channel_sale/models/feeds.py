...
from flectra.addons.flectra_multi_channel_sale.tools import parse_float,extract_list as EL
import logging
    ...
    def import_items(self):
        ...
        for record in self:
            ...
            if mapping_id:
                ...
                sync_vals['flectra_id'] = mapping_id.flectra_category_id
...
class ProductFeed(models.Model):
    ...
    def _create_product_line(self, variant, template_id, store_id, location_id,channel_id):
        ...
        if not exists:
            ...
            product_exists_flectra = channel_id.match_flectra_product(vals)
            if not product_exists_flectra:
                ...
                product_exists_flectra.write(vals)
                product_id = product_exists_flectra
    ...
    def import_product(self,channel_id):
        ...
        template_exists_flectra = channel_id.match_flectra_template(
        ...
        if state == 'done':
            if match:
                ...
                if not template_exists_flectra:
                    template_id = match.template_name
                    template_id.with_context(context).write(vals)
                else:
                    template_exists_flectra.with_context(context).write(vals)
                    template_id = template_exists_flectra
                ...
                try:
                    ...
                    state = 'error'
                if state == 'done':
                    template_id = template_id and template_id or template_exists_flectra
                    if template_id:
    ...
    def import_items(self):
        ...
        for record in self:
            ...
            mapping_id = update_id or create_id
            if mapping_id:
                mapping_vals= mapping_id.read(['store_product_id','flectra_template_id'])[0]
                sync_vals['status'] = 'success'
                sync_vals['ecomstore_refrence'] =mapping_vals.get('store_product_id')
                sync_vals['flectra_id'] = mapping_vals.get('flectra_template_id')
            sync_vals['summary'] = msz
