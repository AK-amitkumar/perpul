...
class ChannelAttributetMappings(models.Model):
	...
	attribute_name = fields.Many2one('product.attribute',string='Perpul Attribute Name', required=True)
	flectra_attribute_id = fields.Integer(string='Perpul Attribute ID',required=True)
	...
	def change_flectra_id(self):
		self.flectra_attribute_id = self.attribute_name.id
...
class ChannelAttributetValueMappings(models.Model):
	...
	attribute_value_name = fields.Many2one('product.attribute.value',string='Perpul Attribute Value Name', required=True)
	flectra_attribute_value_id = fields.Integer('Perpul Value ID',required=True)

	@api.onchange('attribute_value_name')
	def change_flectra_id(self):
		self.flectra_attribute_value_id = self.attribute_value_name.id
	def _compute_name(self):
		for record in self:
