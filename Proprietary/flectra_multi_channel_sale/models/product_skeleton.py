...
class ChannelTemplateMappings(models.Model):
    ...
    store_product_id = fields.Char(
        ...
        required=True)
    flectra_template_id = fields.Char(
        string='Perpul Template ID',
        required=True)
    ...
    _sql_constraints = [
        ...
        # ('channel_flectra_flectra_template_id_uniq',
        #     'unique(channel_id, flectra_template_id)',
        #     'Perpul Template ID must be unique for channel template mapping!'),
    ]
    @api.multi
    def unlink(self):
        for record in self:
    ...
    def change_flectra_id(self):
        self.flectra_template_id = self.template_name.id
...
class ChannelProductMappings(models.Model):
    ...
    flectra_template_id = fields.Many2one(
        related='product_name.product_tmpl_id',
        string='Perpul Template')
    default_code = fields.Char(
        string="Default code/SKU")
    barcode = fields.Char(
        string="Barcode/EAN/UPC or ISBN")
    ...
    def change_flectra_id(self):
        ...
        self.flectra_template_id = self.product_name.product_tmpl_id.id
