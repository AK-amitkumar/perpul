...
from flectra.addons.flectra_multi_channel_sale.tools import chunks,  DomainVals, ReverseDict
...
class MultiChannelSale(models.Model):
    ...
    def display_message(self, message):
        ...
        return {
            'name': _("Summary"),
            'view_mode': 'form',
            'view_id': self.env.ref('flectra_multi_channel_sale.wizard_message_form').id,
            'view_type': 'form',
    ...
    def open_record_view(self):
        self.ensure_one()
        res_model = self._context.get('mapping_model')
        flectra_mapping_field = self._context.get('flectra_mapping_field')
        domain = [
            ('channel_id', '=', self.id),
        ]
        erp_ids = self.env[res_model].search(domain).mapped(flectra_mapping_field)
    ...
    order_state_ids = fields.One2many(
        ...
        string='Default Perpul Order States',
        help='Imported order will process in flectra on basis of these state mappings.',
    )
    ...
    def match_create_pricelist_id(self, currency_id):
        ...
        if match:
            return match.flectra_pricelist_id
        else:
            ...
            vals = dict(
                ...
                store_currency_code=currency_id.name,
                flectra_pricelist_id=pricelist_id.id,
                flectra_currency_id=currency_id.id,
            )
            return self._create_mapping(map_obj, vals).flectra_pricelist_id
    ...
    flectra_attribute_id=None,domain = None, limit=1):

        map_domain = self.get_channel_domain(domain)

        if store_attribute_id:
            map_domain += [('store_attribute_id', '=', store_attribute_id)]
        if flectra_attribute_id:
            map_domain += [('flectra_attribute_id', '=', flectra_attribute_id)]
    ...
    def match_attribute_value_mappings(self, store_attribute_value_id=None,
        ...
        if attribute_value_id:
            map_domain +=   [('flectra_attribute_value_id', '=', attribute_value_id)]
    ...
    @api.model
    def match_category_mappings(self, store_category_id=None,flectra_category_id=None, domain=None, limit=1):
        map_domain = self.get_channel_domain(domain)
        if store_category_id:
            map_domain += [('store_category_id', '=', store_category_id)]
        if flectra_category_id:
            map_domain += [('flectra_category_id', '=', flectra_category_id)]
    ...
    def create_attribute_mapping(self, erp_id, store_id,store_attribute_name=''):
        ...
        if store_id and store_id not in ['0', -1]:
            vals = dict(
                ...
                flectra_attribute_id=erp_id.id,
    ...
    def create_attribute_value_mapping(self, erp_id, store_id,store_attribute_value_name=''):
        ...
        if store_id and store_id not in ['0',' ', -1]:
            vals = dict(
                ...
                flectra_attribute_value_id=erp_id.id,
            )
            channel_vals = self.get_channel_vals()
            vals.update(channel_vals)
    ...
    def create_partner_mapping(self, erp_id, store_id, _type):
        ...
        if store_id and store_id not in ['0', -1]:
            vals = dict(
                ...
                flectra_partner_id=erp_id.id,
                flectra_partner=erp_id.id,
                type=_type,
            )
            channel_vals = self.get_channel_vals()
            vals.update(channel_vals)
    ...
    def create_carrier_mapping(self, name, service_id=None):
        ...
        vals = dict(
            ...
            flectra_carrier_id=carrier_id.id,
            flectra_shipping_carrier=carrier_id.id,
        )
        channel_vals = self.get_channel_vals()
        vals.update(channel_vals)
    ...
    @api.model
    def match_flectra_template(self, vals,variant_lines):
        ...
        if not record:
            ...
            if not record:
	            ...
	            for var in variant_lines:
	                match  =self.match_flectra_product(var.read([])[0])
	                if match:
    ...
    @api.model
    def match_flectra_product(self, vals, obj='product.product'):
        oe_env = self.env[obj]
        record = False
    ...
    @api.model
    def create_product_mapping(self, flectra_template_id, flectra_product_id,
        ...
        vals.update(dict(
            ...
            erp_product_id=flectra_product_id.id,
            product_name=flectra_product_id.id,
            flectra_template_id=flectra_template_id.id,
            default_code=vals.get('default_code'),
            barcode=vals.get('barcode'),
        ))
        channel_vals = self.get_channel_vals()
    ...
    def create_order_mapping(self, erp_id, store_id,store_source=None):
        self.ensure_one()
        vals = dict(
            flectra_partner_id=erp_id.partner_id,
            store_order_id=store_id,
            store_id=store_source,
            flectra_order_id=erp_id.id,
    ...
    def get_channel_category_id(self,template_id,channel_id):
        ...
        if extra_category_ids:
            domain = [('flectra_category_id', 'in',extra_category_ids.ids)]
    ...
    def set_order_by_status(self,channel_id,store_id,
        ...
        if order_match  and order_match.order_name.state =='draft' and (
                state.flectra_create_invoice or state.flectra_ship_order):
            result['message'] += self.env['multi.channel.skeleton']._SetFlectraOrderState(
