...
class ChannelOrderMappings(models.Model):
	...
	flectra_order_id = fields.Integer(string='Perpul Order ID',required=True)
	order_name = fields.Many2one('sale.order',string='Perpul Order')
	flectra_partner_id = fields.Many2one(related='order_name.partner_id')
	...
	def change_flectra_id(self):
		self.flectra_order_id = self.order_name.id
